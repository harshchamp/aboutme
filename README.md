# Hey there! 👋 I'm Harsh

Welcome to my GitHub profile! I'm a passionate data scientist and engineer exploring the intersection of machine learning, IoT, and optimization.

## 🚀 About Me

I'm enthusiastic about building innovative solutions and tackling complex engineering challenges. My work spans across:
- **Data Science & Machine Learning** - A/B Testing, Statistical Analysis, Predictive Modeling
- **IoT & Sensor Systems** - Real-time data collection and analysis
- **Process Optimization** - Manufacturing optimization (Micro-Injection Moulding)
- **Pricing Models** - Dynamic pricing strategies and analysis

## 📚 Featured Projects

- **[A/B Testing Model](https://github.com/harshchamp/A-B-testing-Model)** - Statistical analysis and A/B testing implementations on Gaming data
- **[IoT Sensor](https://github.com/harshchamp/IOT_Sensor)** - IoT sensor data collection and processing
- **[Micro-Injection Moulding](https://github.com/harshchamp/Micro-Injection-Moulding)** - Micro Injection Moulding process optimization and state of art Ensemble Model use
- **[Solar Hexagon](https://github.com/harshchamp/solar_hexagon)** - Solar energy analysis project for a startup
- **[Pricing](https://github.com/harshchamp/Pricing)** - Pricing strategy models 

## 🛠️ Tech Stack

- **Languages**: Python, SQL, Jupyter Notebook
- **Tools**: Data Science, Machine Learning, Statistical Analysis
- **Domains**: IoT, Finance, Manufacturing, E-commerce, Energy

## 💡 What I'm Working On

Currently exploring new opportunities to apply data-driven insights and build scalable solutions. Always interested in collaborating on interesting projects!

## 📫 Let's Connect

Feel free to explore my repositories and reach out if you'd like to collaborate or discuss ideas!
**[My LinkedIn](https://www.linkedin.com/in/harsh-raj-6b3674128/)** 

---

⭐️ If you find my work interesting, don't forget to star my repositories!
